 // Toda a lógica de JavaScript foi mantida e não precisa de alterações.
    ...
    function renderMenu() {
        ...
        if (produtosVisiveis.length === 0) { Object.values(containers).forEach(c => c.innerHTML = '<p class="text-red-500 text-sm col-span-full">Nenhum item. Faça login como admin para adicionar produtos.</p>'); return; }
    ...
    function renderConfigAdmin() {
        ...
        document.getElementById('content-config').innerHTML = `<div class="bg-white p-6 rounded-2xl shadow-lg"><h3 class="text-2xl font-semibold mb-4 text-purple-700">Configurações Gerais</h3><div class="mb-6 p-4 border border-gray-200 rounded-lg"><label for="whatsapp-number" class="block font-semibold mb-2">Número do WhatsApp para Pedidos</label><input type="text" id="whatsapp-number" placeholder="Ex: 5511999998888" class="w-full p-2 border rounded border-gray-300"></div><h3 class="text-2xl font-semibold mb-4 text-purple-700">Horário de Funcionamento</h3><div class="p-4 border border-gray-200 rounded-lg">${diasHTML}</div><h3 class="text-2xl font-semibold mt-6 mb-4 text-purple-700">Mensagem (Loja Fechada)</h3><textarea id="mensagem-fechado" class="w-full p-2 border rounded border-gray-300" rows="3" placeholder="Ex: Estamos fechados. Nosso horário é de..."></textarea><button id="salvar-config-btn" class="bg-green-500 text-white p-2 rounded hover:bg-green-600 mt-4">Salvar Configurações</button></div>`;
    ...
    function carregarVendasAdmin(startDate, endDate) {
        ...
        unsubscribeVendas = onSnapshot(q, (snapshot) => {
            if (initialVendasLoadComplete && snapshot.docChanges().some(change => change.type === 'added')) { playNotificationSound(); showToast("Novo Pedido Recebido!"); document.getElementById('tab-vendas').click(); }
            tableBody.innerHTML = ''; 
            let totalVendas = 0;
            ...
            if (snapshot.empty) { 
                ...
                document.getElementById('total-por-tamanho').innerHTML = '';
            } else {
                snapshot.docs.forEach(docSnap => {
                    ...
                    if (!isNaN(valorNumerico)) { 
                        ...
                        // Adiciona a contagem por tamanho de copo
                        if (venda.tamanho && !venda.pedidoCombo) {
                            if (!totaisPorTamanho[venda.tamanho]) {
                                totaisPorTamanho[venda.tamanho] = { count: 0, total: 0 };
                            }
                ...
                if (Object.keys(totaisPorTamanho).length > 0) {
                    ...
                    totaisHTML += '<p class="text-sm text-gray-500">Nenhum copo vendido no período.</p>';
                }
    ...
    async function salvarConfiguracoes() {
        const dias = ['domingo', 'segunda', 'terca', 'quarta', 'quinta', 'sexta', 'sabado'];
        const settings = { mensagemFechado: document.getElementById('mensagem-fechado').value, whatsappNumber: document.getElementById('whatsapp-number').value };
    ...
    async function carregarConfiguracoesAdmin() {
        ...
        if (docSnap.exists()) {
            ...
            document.getElementById('whatsapp-number').value = settings.whatsappNumber || ''; document.getElementById('mensagem-fechado').value = settings.mensagemFechado || '';
            const dias = ['domingo', 'segunda', 'terca', 'quarta', 'quinta', 'sexta', 'sabado'];
    ...
    function carregarFluxoCaixa(startDate, endDate) {
        ...
        unsubscribeFluxoCaixa = onSnapshot(q, (snapshot) => {
            ...
            document.getElementById('total-entradas').innerText = `R$${totalEntradas.toFixed(2).replace('.', ',')}`; document.getElementById('total-saidas').innerText = `R$${totalSaidas.toFixed(2).replace('.', ',')}`; document.getElementById('saldo-atual').innerText = `R$${(totalEntradas - totalSaidas).toFixed(2).replace('.', ',')}`;
    ...
    function checkStoreOpen() {
        ...
        [sendOrderBtnMobile, sendOrderBtnDesktop].forEach(btn => {
            if (btn) {
                ...
                avisoLojaFechada.classList.toggle('hidden', isStoreOpen);
                if (!isStoreOpen) { msgLojaFechada.innerText = storeSettings.mensagemFechado || "Estamos fechados no momento."; }
            }
        });
    }
    ...
    }, (error) => { console.error("Erro ao carregar produtos:", error); document.getElementById('menu-container').innerHTML = '<p class="text-red-500 text-center">Não foi possível carregar o cardápio.</p>'; });
